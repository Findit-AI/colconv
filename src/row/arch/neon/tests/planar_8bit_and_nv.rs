...
// (`out[j] = (prev[j] + cur[j] + 1) >> 1`), scalar vs the NEON `vrhaddq`
...
fn check_440_bottom_v_equivalence(width: usize) {
  ...
  let mut out_neon = std::vec![0u8; width];
  scalar::chroma_upsample_440_bottom_v(&prev, &cur, &mut out_scalar, width);
  unsafe {
    chroma_upsample_440_bottom_v(&prev, &cur, &mut out_neon, width);
  }
  assert_eq!(
    out_scalar, out_neon,
    "NEON 440 bottom-v u8 diverges (width={width})"
  );
}
...
fn check_440_bottom_v_u16_equivalence(width: usize) {
  ...
  let mut out_neon = std::vec![0u16; width];
  scalar::chroma_upsample_440_bottom_v_u16(&prev, &cur, &mut out_scalar, width);
  unsafe {
    chroma_upsample_440_bottom_v_u16(&prev, &cur, &mut out_neon, width);
  }
  assert_eq!(
    out_scalar, out_neon,
    "NEON 440 bottom-v u16 diverges (width={width})"
  );
}
...
#[test]
#[cfg_attr(miri, ignore = "NEON SIMD intrinsics unsupported by Miri")]
fn neon_440_bottom_v_matches_scalar() {
  for w in [
