...
/// [`scalar::chroma_upsample_440_bottom_v`]:
/// `out[j] = (prev[j] + cur[j] + 1) >> 1` for every column, via `vrhaddq_u8`
/// (16 lanes per iteration) with a scalar tail.
///
/// # Safety
///
/// 1. **NEON must be available on the current CPU** (baseline on aarch64; the
///    dispatcher verifies it with `is_aarch64_feature_detected!("neon")` for
///    the general case).
...
/// NEON `u16` twin of [`chroma_upsample_440_bottom_v`], byte-identical to
/// [`scalar::chroma_upsample_440_bottom_v_u16`] on host-native `u16` chroma:
/// `out[j] = (prev[j] + cur[j] + 1) >> 1` via `vrhaddq_u16` (8 lanes per
/// iteration) with a scalar tail.
///
/// # Safety
///
/// 1. **NEON must be available on the current CPU** (baseline on aarch64).
/// 2. `prev.len() >= width`, `cur.len() >= width`, `out.len() >= width`.
#[inline]
#[target_feature(enable = "neon")]
#[cfg_attr(not(all(test, feature = "std")), allow(dead_code))]
pub(crate) unsafe fn chroma_upsample_440_bottom_v_u16(
  ...
  unsafe {
    while j + 8 <= width {
      let p = vld1q_u16(prev.as_ptr().add(j));
      let c = vld1q_u16(cur.as_ptr().add(j));
      vst1q_u16(out.as_mut_ptr().add(j), vrhaddq_u16(p, c));
      j += 8;
    }
  }
  while j < width {
