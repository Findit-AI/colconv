...
/// U[k] = 2k + 1 (one chroma sample per 2 pixels at 4:2:2) for a
/// V210 6-pixel group. V = 512 (neutral midpoint, bias-subtracted = 0).
///
/// Asserts:
/// - `luma_u16_row` output = [1, 2, 3, 4, 5, 6] (Y values direct, no shift)
...
fn wasm_simd128_v210_lane_order_per_pixel_y_and_u() {
  ...
  unsafe { v210_to_luma_u16_row(&packed, &mut luma_out, W); }
  let expected_luma: std::vec::Vec<u16> = (1..=W as u16).collect();
  assert_eq!(luma_out, expected_luma, "wasm_simd128 v210 luma reorder bug");
  ...
  assert_eq!(simd_rgb, scalar_rgb, "wasm_simd128 v210 SIMD vs scalar diverges — chroma deinterleave bug");
}
