...
// (`out[j] = (prev[j] + cur[j] + 1) >> 1`), scalar vs the AVX-512 `_mm512_avg`
...
fn check_440_bottom_v_equivalence(width: usize) {
  ...
  let mut out_avx512 = std::vec![0u8; width];
  scalar::chroma_upsample_440_bottom_v(&prev, &cur, &mut out_scalar, width);
  unsafe {
    chroma_upsample_440_bottom_v(&prev, &cur, &mut out_avx512, width);
  }
  assert_eq!(
    out_scalar, out_avx512,
    "AVX-512 440 bottom-v u8 diverges (width={width})"
  );
}
...
fn check_440_bottom_v_u16_equivalence(width: usize) {
  ...
  let mut out_avx512 = std::vec![0u16; width];
  scalar::chroma_upsample_440_bottom_v_u16(&prev, &cur, &mut out_scalar, width);
  unsafe {
    chroma_upsample_440_bottom_v_u16(&prev, &cur, &mut out_avx512, width);
  }
  assert_eq!(
    out_scalar, out_avx512,
    "AVX-512 440 bottom-v u16 diverges (width={width})"
  );
}

#[cfg(feature = "yuv-planar")]
#[test]
fn avx512_440_bottom_v_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx512bw") {
    return;
  }
  for w in [
