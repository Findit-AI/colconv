...
/// `out[j] = (prev[j] + cur[j] + 1) >> 1` via `_mm_avg_epu8` (16 lanes per
/// iteration) with a scalar tail.
///
/// # Safety
///
/// 1. **SSE4.1 must be available on the current CPU** (the dispatcher verifies
///    it with `is_x86_feature_detected!("sse4.1")`).
...
/// SSE4.1 `u16` twin of [`chroma_upsample_440_bottom_v`], byte-identical to
...
/// `_mm_avg_epu16` (8 lanes per iteration) with a scalar tail.
///
/// # Safety
///
/// 1. **SSE4.1 must be available on the current CPU.**
...
pub(crate) unsafe fn chroma_upsample_440_bottom_v_u16(
  ...
  unsafe {
    while j + 8 <= width {
      let p = _mm_loadu_si128(prev.as_ptr().add(j).cast());
      let c = _mm_loadu_si128(cur.as_ptr().add(j).cast());
      _mm_storeu_si128(out.as_mut_ptr().add(j).cast(), _mm_avg_epu16(p, c));
      j += 8;
    }
  }
  while j < width {
