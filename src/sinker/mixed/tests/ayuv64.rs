...
fn ayuv64_with_rgb_smoke() {
  ...
  ayuv64_to(&src, false, sink.set_kernel_matrix(KernelMatrix::Bt709)).unwrap();
...
fn ayuv64_with_rgb_and_rgba_preserves_source_alpha() {
  ...
  let mut sinker = MixedSinker::<Ayuv64>::new(width, height)
    ...
    .with_rgba(&mut rgba)
    .unwrap();
  ayuv64_to(&frame, true, sinker.set_kernel_matrix(KernelMatrix::Bt709)).unwrap();
...
fn ayuv64_planar_parity_with_yuva444p16() {
  ...
  ayuv64_to(
    &packed_frame,
    full_range,
    a_sink.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
  ...
  ayuv64_to(
    &packed_frame,
    full_range,
    a_sink2.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
  ...
  yuva444p16_to(
    &planar,
    full_range,
    p_sink3.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
  ...
  ayuv64_to(
    &packed_frame,
    full_range,
    a_sink3.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
  ...
  yuva444p16_to(
    &planar,
    full_range,
    p_sink4.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
  ...
  ayuv64_to(
    &packed_frame,
    full_range,
    a_sink4.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
    ...
    for matrix in [
      ...
      let mut sinker_rgba = std::vec![0u8; width * height * 4];
      {
        let mut sink = MixedSinker::<Ayuv64>::new(width, height)
          ...
          .unwrap();
        ayuv64_to(&frame, full_range, sink.set_kernel_matrix(matrix)).unwrap();
      }
...
fn ayuv64_le_be_roundtrip_byte_identical() {
  ...
  ayuv64_to(
    &frame_le,
    true,
    sink_le_u16.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
  ...
  ayuv64_to_endian(
    &frame_be,
    true,
    sink_be_u16.set_kernel_matrix(KernelMatrix::Bt709),
  )
  .unwrap();
...
fn ayuv64_rgb_scratch_alloc_failure_leaves_outputs_untouched() {
  ...
  let err = ayuv64_to(&src, false, sink.set_kernel_matrix(KernelMatrix::Bt601)).unwrap_err();
  drop(sink);
