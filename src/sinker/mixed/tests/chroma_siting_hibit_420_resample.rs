...
//! 4:2:0 YUV family — `Yuv420p9` … `Yuv420p16` (RFC #238 S6a), the `u16` twin of
//! the 8-bit `chroma_siting_420_resample` suite and the 4:2:0 sibling of the
//! high-bit `chroma_siting_hibit_422_resample` suite.
//!
//! 4:2:0 subsamples chroma 2:1 horizontally AND vertically. S6a routes the
...
//!    triangle into the [`ResamplePlan::area_chroma_420`] chroma weights and
...
//!    source row (each luma row `r` takes chroma row `r / 2`, horizontally
//!    upsampled) then convert-then-bin — pinned against a `Yuv444pN` resample of
//!    the independently reconstructed planes.
//!
...
//! rounding), never a tolerance. Source heights are EVEN so the plan's
...
/// with a SINGLE round-half-up. The vertical mirror of the bottom-sited native
/// oracle / the `u16` twin of the 8-bit `bin_chroma_top`.
...
/// RGB-domain oracle's reconstruction step (mirror of the co-sited-vertical
/// reconstruction in [`rgb_domain_oracle`]).
...
/// the co-sited chroma box). Widths are even (Yuv420p requires it).
const GEOMS: [(usize, usize, usize, usize); 4] =
  [(8, 8, 4, 4), (8, 8, 5, 3), (12, 8, 4, 4), (16, 8, 6, 5)];

macro_rules! hibit_420_resample_siting {
  (
    $mod:ident, $bits:expr,
    $M420:ident, $F420:ident, $w420:ident,
    $M420be:ty, $F420be:ident, $w420be:ident,
    ...
    mod $mod {
      ...
      /// Drive an LE `Yuv420pN` area resample for `rgb` (u8) + `rgb_u16`.
      ...
      /// Drive an LE `Yuv420pN` single-kernel filter resample (Triangle).
      #[allow(clippy::too_many_arguments)]
      fn run_filter(
        y: &[u16],
        u: &[u16],
      ...
      /// Drive a BE `Yuv420pN` area resample (planes re-encoded BE-wire).
      #[allow(clippy::too_many_arguments)]
      fn run_be(
        y: &[u16],
        u: &[u16],
      ...
      /// the exact centered chroma oracle to `ow x oh`, then convert ONCE at
      ...
      /// with the #302 kernel — each luma row `r` taking chroma row `r / 2` (the
      /// co-sited vertical replication) — then run that full-resolution
      ...
      // ==== RFC #238 S6d: bottom-sited (v = 1) VERTICAL fold ================
      ...
      /// The bottom-sited NATIVE oracle: bin Y co-sited and U / V through the
      /// exact bottom V-fold oracle to `ow x oh`, then convert ONCE at output
      ...
      /// the bottom-sited decode — the identity bottom kernel at source width:
      /// even rows box-blend chroma rows `i - 1` (clamped) and `i` (round-half-up),
      /// odd rows take chroma row `i`, each then horizontally upsampled with the
      /// #302 centered kernel. The shared reconstruction step for every
      /// reconstruct-then-bin bottom oracle.
      ...
      /// the vertical bottom blend then run that `Yuv444pN` frame through the
      ...
      /// Direct (non-resample, identity dims) `Yuv420pN` `Bottom` decode — the
      /// delay-line kernel path, the already-validated identity reference the
      /// resample path must match at identity dimensions.
      fn direct_bottom(
        y: &[u16],
        u: &[u16],
        v: &[u16],
      ...
      fn bottom_identity_resample_matches_direct_decode() {
        ...
        // chroma with the SAME bottom kernel as the direct (non-resample) decode
      ...
      /// Direct (non-resample, identity dims) `Yuv420pN` `Top` decode — the
      /// forward-delay-line kernel path, the already-validated identity reference
      /// the resample path must match at identity dimensions.
      fn direct_top(
        y: &[u16],
        u: &[u16],
        v: &[u16],
      ...
      fn top_row_stage_equals_rgb_reconstruct_then_bin() {
        ...
        for (sw, sh, ow, oh) in GEOMS {
          ...
          assert_eq!(
            ...
            o,
            "top row-stage must equal the RGB-domain reconstruct-then-bin oracle \
             ({sw}x{sh}->{ow}x{oh})"
          );
        }
      }
      ...
      fn top_filter_equals_rgb_reconstruct_then_bin() {
        for (sw, sh, ow, oh) in GEOMS {
          ...
          assert_eq!(
            ...
            o,
            "top filter must equal the RGB-domain Triangle oracle ({sw}x{sh}->{ow}x{oh})"
          );
        }
      }
      ...
      fn bottom_final_even_row_reserve_alloc_failure_retries_with_blend() {
        ...
        // the row (#180), so a rejected row leaves the lookback untouched.
