//! Chroma-siting-aware fused-downscale coverage for the HIGH-BIT **packed** 4:2:2
//! Y2xx family — `Y210` (10-bit), `Y212` (12-bit), `Y216` (16-bit), the packed
//! twin of the planar `chroma_siting_hibit_422_resample` and the semi-planar
//! `chroma_siting_hibit_422_semiplanar_resample`.
//!
//! 4:2:2 subsamples chroma 2:1 horizontally only. The packed `Y₀ U Y₁ V` words
//! are de-interleaved to the SAME half-width U / V planes a `Yuv422pN` frame
//! holds, so every centered packed resample is **bit-identical** to the centered
//! `Yuv422pN` resample of those planes — the strongest catch for a U/V swap in the
//! de-interleave. Routing the centered siting (`Center` / `Top` / `Bottom`,
//! [`chroma_422_center_sited_h`]) through the resample means:
...
//!    **single-kernel filter tier** reconstruct full-width `u16` chroma per source
//!    row then convert-then-bin 4:4:4 — the RGB-domain reconstruct-then-bin, pinned
//!    against a `Yuv444pN` resample of the independently reconstructed planes.
//!
//! The Y2xx wire is MSB-aligned (`logical << (16 - BITS)`); the logical values
//! decode identically to the low-packed `Yuv422pN` / `Yuv444pN` oracle planes, so
//! the siting math is depth- and endian-independent (pinned by the BE↔LE parity
//! test). Every centered assertion is an EXACT match (single rounding), never a
//! tolerance.
...
use crate::{
  ...
  source::{
    Y210, Y210Row, Y212, Y212Row, Y216, Y216Row, Yuv422p10, Yuv422p12, Yuv422p16, Yuv444p10,
    Yuv444p12, Yuv444p16, y210_to, y210_to_endian, y212_to, y212_to_endian, y216_to,
    y216_to_endian, yuv422p10_to, yuv422p12_to, yuv422p16_to, yuv444p10_to, yuv444p12_to,
    yuv444p16_to,
  },
};
use mediaframe::frame::{
  Yuv422p10Frame, Yuv422p12Frame, Yuv422p16Frame, Yuv444p10Frame, Yuv444p12Frame, Yuv444p16Frame,
};
...
macro_rules! hibit_422_packed_resample_siting {
  (
    $mod:ident, $bits:expr,
    $M:ident, $Row:ident, $w:ident, $wbe:ident,
    ...
    mod $mod {
      ...
      /// MSB-align a LOGICAL sample into the Y2xx wire `u16`: `value << (16 -
      /// BITS)` (Y210 `<< 6`, Y216 `<< 0`).
      fn pack(v: u16) -> u16 {
        v << SHIFT
      }

      /// Pack the half-width logical U / V and full-width logical Y into the
      /// MSB-aligned packed `Y₀ U Y₁ V` wire (`2·SRC` u16 per row, full-height —
      /// one chroma sample per 2-pixel group).
      fn pack_packed(y: &[u16], u: &[u16], v: &[u16]) -> Vec<u16> {
        let mut buf = vec![0u16; SRC * 2 * SRC];
        for row in 0..SRC {
          for cx in 0..CW {
            let base = row * 2 * SRC + cx * 4;
            buf[base] = pack(y[row * SRC + cx * 2]);
            buf[base + 1] = pack(u[row * CW + cx]);
            buf[base + 2] = pack(y[row * SRC + cx * 2 + 1]);
            buf[base + 3] = pack(v[row * CW + cx]);
          }
        }
        buf
      }
      ...
      /// vertical mistake would show). Returns LOGICAL planes.
      ...
      /// Drive an LE `Y2xx` area resample for `rgb` (u8) + `rgb_u16`, at `loc`
      /// siting and `native` tier.
      fn run(
        y: &[u16],
        u: &[u16],
        v: &[u16],
      ...
      /// Drive an LE `Y2xx` single-kernel filter resample (Triangle).
      fn run_filter(
        y: &[u16],
        u: &[u16],
        v: &[u16],
      ...
      /// Drive a BE `Y2xx` area resample (wire re-encoded BE).
      fn run_be(
        y: &[u16],
        u: &[u16],
        v: &[u16],
        ...
        let packed = as_be(&pack_packed(y, u, v));
        let mut rgb = vec![0u8; OUT * OUT * 3];
        let mut rgb16 = vec![0u16; OUT * OUT * 3];
        {
          let mut sink = MixedSinker::<$M<true>, AreaResampler>::with_resampler(
            SRC,
            SRC,
      ...
      /// through a planar 4:2:2 resample. A centered packed decode must equal this
      /// byte-for-byte — the de-interleave / U-V-swap catch.
      fn run_yuv422p(
        y: &[u16],
        u: &[u16],
        v: &[u16],
        ...
        for loc in [
          ...
          for native in [true, false] {
            assert_eq!(
              ...
              "centered packed {loc:?} must equal centered planar Yuv422p (native={native})"
            );
          }
        }
      }
...
hibit_422_packed_resample_siting!(
  y210,
  10,
  Y210,
  Y210Row,
