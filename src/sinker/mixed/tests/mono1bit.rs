...
fn monoblack_walker_to_rgb() {
  ...
  let mut rgb = vec![0u8; 8 * 2 * 3];
  {
    let mut sinker = MixedSinker::<Monoblack>::new(8, 2)
      .with_rgb(&mut rgb)
      .expect("attach rgb");
    monoblack_to(&frame, true, sinker.set_kernel_matrix(KernelMatrix::Bt709)).expect("walk ok");
  }
...
fn monowhite_walker_to_luma() {
  ...
  let mut luma = vec![0u8; 8];
  {
    let mut sinker = MixedSinker::<Monowhite>::new(8, 1)
      .with_luma(&mut luma)
      .expect("attach luma");
    monowhite_to(&frame, true, sinker.set_kernel_matrix(KernelMatrix::Bt709)).expect("walk ok");
  }
