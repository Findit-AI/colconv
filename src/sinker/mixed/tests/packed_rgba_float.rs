...
fn rgbaf32_simd_matches_scalar_with_random_input() {
  ...
  let mut s_simd = MixedSinker::<Rgbaf32>::new(w, h)
    ...
    .unwrap();
  rgbaf32_to(&src, true, KernelMatrix::Bt709, &mut s_simd).unwrap();
  ...
  rgbaf32_to(&src, true, KernelMatrix::Bt709, &mut s_scalar).unwrap();
...
fn rgbaf32_le_be_roundtrip_byte_identical() {
  ...
  let mut sink_le = MixedSinker::<Rgbaf32>::new(16, 4)
    ...
    .unwrap();
  rgbaf32_to(&frame_le, true, KernelMatrix::Bt709, &mut sink_le).unwrap();
  ...
  let mut sink_be = MixedSinker::<Rgbaf32<true>>::new(16, 4)
    ...
    .unwrap();
  rgbaf32_to_endian(&frame_be, true, KernelMatrix::Bt709, &mut sink_be).unwrap();
