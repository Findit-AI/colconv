  ...
  );
  {
    let mut sink =
      MixedSinker::<Bgr24, AreaResampler>::with_resampler(SRC, SRC, AreaResampler::to(OUT, OUT))
        ...
        .unwrap();
    bgr24_to(&bgr_src, true, KernelMatrix::Bt709, &mut sink).unwrap();
  }
  ...
  );
  {
    let mut sink =
      MixedSinker::<Rgb24, AreaResampler>::with_resampler(SRC, SRC, AreaResampler::to(OUT, OUT))
        ...
        .unwrap();
    rgb24_to(&rgb_src, true, KernelMatrix::Bt709, &mut sink).unwrap();
  }
...
fn bgr24_resample_no_outputs_is_a_no_op() {
  ...
  sink
    .process(Bgr24Row::new(row2, 2, KernelMatrix::Bt709, true))
    .unwrap();
}
...
fn bgr24_filter_plan_rejected_with_typed_error() {
  ...
  .with_rgb(&mut rgb)
  .unwrap();
  let err = bgr24_to(&src, true, KernelMatrix::Bt709, &mut sink).unwrap_err();
  assert!(
    matches!(
      err,
...
fn bgr24_resample_rejects_out_of_sequence_rows() {
  ...
  let err = sink
    .process(Bgr24Row::new(row1, 1, KernelMatrix::Bt709, true))
    .unwrap_err();
  assert!(
    matches!(
      err,
...
fn bgr24_first_row_scratch_oom_leaves_stream_and_freeze_uncommitted_for_retry() {
  ...
  let mut luma = vec![0u8; OUT * OUT];
  {
    ...
    let err = sink
      .process(Bgr24Row::new(&buf[..SRC * 3], 0, KernelMatrix::Bt709, true))
      .unwrap_err();
    assert!(
      matches!(
        err,
