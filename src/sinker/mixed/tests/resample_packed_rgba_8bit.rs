  ...
  ($modname:ident, $marker:ident, $row:ident, $walk:ident, $frame:ident, $perm:expr) => {
    mod $modname {
      ...
      fn straight_rgb_luma_hsv_derive_from_binned_color() {
        ...
        let mut ref_v = std::vec![0u8; OUT * OUT];
        {
          let mut sink = MixedSinker::<crate::source::Rgb24>::new(OUT, OUT)
            ...
            .unwrap();
          crate::source::rgb24_to(&rgb_src, true, sink.set_kernel_matrix(KernelMatrix::Bt709)).unwrap();
        }
      ...
      fn no_output_sink_is_a_noop() {
        ...
        sink.process($row::for_tests(row3, 3, KernelMatrix::Bt709, true)).unwrap();
      }
      ...
      fn rejects_mid_frame_output_change() {
        ...
        sink.begin_frame(SRC as u32, SRC as u32).unwrap();
        sink
          .process($row::for_tests(&pix[..SRC * 4], 0, KernelMatrix::Bt709, true))
          .unwrap();
        sink.set_luma(&mut luma).unwrap();
        let err = sink
          .process($row::for_tests(&pix[SRC * 4..2 * SRC * 4], 1, KernelMatrix::Bt709, true))
          .unwrap_err();
      ...
      fn accepts_alpha_mode_change_across_frames() {
        ...
        for y in 0..SRC {
          sink
            .process($row::for_tests(&pix[y * SRC * 4..(y + 1) * SRC * 4], y, KernelMatrix::Bt709, true))
            .unwrap();
        }
      ...
      fn rejects_route_switch_after_out_of_sequence_first_row() {
        ...
        sink
          .process($row::for_tests(&pix[3 * SRC * 4..4 * SRC * 4], 3, KernelMatrix::Bt709, true))
          .unwrap_err();
      ...
      fn filter_plan_is_accepted_not_unsupported() {
        ...
        let mut rgba = std::vec![0u8; OUT * OUT * 4];
        {
          ...
          .with_rgba(&mut rgba)
          .unwrap();
          $walk(&src, true, sink.set_kernel_matrix(KernelMatrix::Bt709))
