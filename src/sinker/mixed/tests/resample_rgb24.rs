...
fn rgb24_derived_outputs_come_from_binned_rgb() {
  ...
  let mut ref_rgba = vec![0u8; OUT * OUT * 4];
  {
    ...
    let mut sink = MixedSinker::<Rgb24>::new(OUT, OUT)
      ...
      .unwrap();
    rgb24_to(&binned, true, KernelMatrix::Bt709, &mut sink).unwrap();
  }
...
fn rgb24_contracts_hold_on_the_fused_path() {
  ...
  let mut rgb = vec![0u8; OUT * OUT * 3];
  {
    ...
    let err = sink
      .process(Rgb24Row::new(row0, 3, KernelMatrix::Bt709, true))
      .unwrap_err();
    assert!(matches!(
      err,
  ...
  let mut luma = vec![0u8; OUT * OUT];
  {
    ...
    sink.begin_frame(SRC as u32, SRC as u32).unwrap();
    sink
      .process(Rgb24Row::new(row0, 0, KernelMatrix::Bt709, true))
      .unwrap();
...
fn rgb24_rejected_first_row_does_not_poison_output_retry() {
  ...
  sink.set_luma(&mut luma).unwrap();
  sink
    .process(Rgb24Row::new(&buf[..SRC * 3], 0, KernelMatrix::Bt709, true))
