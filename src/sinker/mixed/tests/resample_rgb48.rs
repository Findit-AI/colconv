...
fn rgb48_contracts_hold_on_the_fused_path() {
  ...
  let mut rgb_u16 = vec![0u16; OUT * OUT * 3];
  {
    ...
    let err = sink
      .process(Rgb48Row::new(row0, 3, KernelMatrix::Bt709, true))
      .unwrap_err();
    assert!(matches!(
      err,
  ...
  let mut luma = vec![0u8; OUT * OUT];
  {
    ...
    sink.begin_frame(SRC as u32, SRC as u32).unwrap();
    sink
      .process(Rgb48Row::new(row0, 0, KernelMatrix::Bt709, true))
      .unwrap();
  ...
  let mut rgb_u16 = vec![0u16; OUT * OUT * 3];
  {
    let mut sink =
      MixedSinker::<Bgr48, AreaResampler>::with_resampler(SRC, SRC, AreaResampler::to(OUT, OUT))
        ...
        .with_rgb_u16(&mut rgb_u16)
        .unwrap();
    bgr48_to(&src, true, KernelMatrix::Bt709, &mut sink).unwrap();
  }
  let swap = [2usize, 1, 0];
  for oy in 0..OUT {
    for ox in 0..OUT {
...
fn rgb48_u16_only_downscale_does_not_size_the_narrow_scratch() {
  ...
  let mut sink2 =
    MixedSinker::<Rgb48, AreaResampler>::with_resampler(SRC, SRC, AreaResampler::to(OUT, OUT))
      ...
      .with_rgb(&mut rgb)
      .unwrap();
  rgb48_to(&src, true, KernelMatrix::Bt709, &mut sink2).unwrap();
  assert!(
...
fn rgb48_out_of_sequence_first_row_rejected_before_allocation() {
  ...
  sink.begin_frame(SRC as u32, SRC as u32).unwrap();
  let err = sink
    .process(Rgb48Row::new(row3, 3, KernelMatrix::Bt709, true))
    .unwrap_err();
  assert!(
    matches!(
      err,
