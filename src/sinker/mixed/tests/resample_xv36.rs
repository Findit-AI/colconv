...
fn direct_full(packed: &[u16]) -> (Vec<u8>, Vec<u16>, Vec<u16>) {
  ...
  let mut y_u16 = vec![0u16; SRC * SRC];
  {
    let mut sink = MixedSinker::<Xv36>::new(SRC, SRC)
      ...
      .unwrap();
    xv36_to(&src, FR, sink.set_kernel_matrix(M)).unwrap();
  }
...
fn xv36_uniform_gray_downscale_leaves_colour_outputs_unchanged() {
  ...
  let mut vv = vec![0u8; OUT * OUT];
  {
    ...
    .unwrap();
    xv36_to(&xv36_frame(&packed), FR, sink.set_kernel_matrix(M)).unwrap();
  }
...
fn xv36_resample_le_be_parity() {
  ...
  for simd in [true, false] {
    ...
    let mut le_luma_u16 = vec![0u16; OUT * OUT];
    {
      ...
      .unwrap();
      xv36_to(&frame, FR, sink.set_kernel_matrix(M)).unwrap();
    }
    ...
    let mut be_luma_u16 = vec![0u16; OUT * OUT];
    {
      ...
      .unwrap();
      xv36_to_endian(&frame, FR, sink.set_kernel_matrix(M)).unwrap();
    }
...
fn xv36_resets_streams_across_frames() {
  ...
  let mut rgb_u16 = vec![0u16; OUT * OUT * 3];
  {
    ...
    .with_rgb_u16(&mut rgb_u16)
    .unwrap();
    xv36_to(&xv36_frame(&p1), FR, sink.set_kernel_matrix(M)).unwrap();
    xv36_to(&xv36_frame(&p2), FR, sink.set_kernel_matrix(M)).unwrap();
  }
...
fn xv36_out_of_sequence_first_row_rejected_before_allocation() {
  ...
  let err = sink
    .process(Xv36Row::for_tests(row3, 3, M, FR))
    .unwrap_err();
  assert!(
    matches!(
      err,
...
fn xv36_rejects_mid_frame_output_change() {
  ...
  sink.begin_frame(SRC as u32, SRC as u32).unwrap();
  sink
    .process(Xv36Row::for_tests(&packed[..stride], 0, M, FR))
    .unwrap();
  sink.set_luma_u16(&mut luma_u16).unwrap();
  let err = sink
    .process(Xv36Row::for_tests(&packed[stride..2 * stride], 1, M, FR))
    .unwrap_err();
  assert!(
