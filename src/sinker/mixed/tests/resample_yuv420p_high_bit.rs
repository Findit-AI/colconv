    ...
    mod $mod {
      ...
      fn luma_masks_dirty_high_bit_y() {
        ...
        for native in [true, false] {
          let mut luma = std::vec![0u8; OUT * OUT];
          {
            ...
            .with_luma(&mut luma)
            .unwrap();
            $walker(&frame(&y_le, &u_le, &v_le), FR, sink.set_kernel_matrix(M)).unwrap();
          }
          ...
          let mut be_luma = std::vec![0u8; OUT * OUT];
          {
            ...
            .unwrap();
            $walker_be::<_, true>(&frame_be(&y_be, &u_be, &v_be), FR, sink.set_kernel_matrix(M)).unwrap();
          }
      ...
      fn luma_is_native_y_area_mean() {
        ...
        for full_range in [true, false] {
          let mut luma = vec![0u8; OUT * OUT];
          {
            ...
            .with_luma(&mut luma)
            .unwrap();
            $walker(&frame(&y, &u, &v), full_range, sink.set_kernel_matrix(M)).unwrap();
          }
          assert_eq!(
            luma, luma_ref,
      ...
      fn resets_streams_across_frames() {
        ...
        let mut luma = vec![0u8; OUT * OUT];
        {
          ...
          .with_luma(&mut luma)
          .unwrap();
          $walker(&frame(&y1, &u1, &v1), FR, sink.set_kernel_matrix(M)).unwrap();
          $walker(&frame(&y2, &u2, &v2), FR, sink.set_kernel_matrix(M)).unwrap();
        }
        assert_eq!(
          rgb_u16,
      ...
      fn rejects_mid_frame_output_change() {
        ...
        let err = sink
          .process($row::for_tests(&y[SRC..2 * SRC], &u[..CW], &v[..CW], 1, M, FR))
          .unwrap_err();
        assert!(
