...
fn y210_matches_v210_with_same_logical_samples() {
  ...
  v210_to(
    &v210_src,
    true,
    s_v210.set_kernel_matrix(KernelMatrix::Bt2020Ncl),
  )
  .unwrap();
  y210_to(
    &y210_src,
    true,
    s_y210.set_kernel_matrix(KernelMatrix::Bt2020Ncl),
  )
  .unwrap();
...
fn y210_rgb_scratch_alloc_failure_leaves_outputs_untouched() {
  ...
  let err = y210_to(&src, false, sink.set_kernel_matrix(KernelMatrix::Bt601)).unwrap_err();
  drop(sink);
