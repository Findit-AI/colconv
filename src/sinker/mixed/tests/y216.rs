...
fn y216_planar_parity_with_yuv422p16() {
  ...
  .unwrap();
  y216_to(&y216, false, y_sink.set_kernel_matrix(KernelMatrix::Bt709)).unwrap();
  ...
  .unwrap();
  y216_to(&y216, false, y_sink2.set_kernel_matrix(KernelMatrix::Bt709)).unwrap();
...
fn y216_rgb_scratch_alloc_failure_leaves_outputs_untouched() {
  ...
  let err = y216_to(&src, false, sink.set_kernel_matrix(KernelMatrix::Bt601)).unwrap_err();
  drop(sink);
