...
#[cfg(all(feature = "y2xx", feature = "yuv-planar"))]
use crate::row::{yuv444p12_to_rgb_row_endian, yuv444p12_to_rgb_u16_row_endian};
use crate::{
  PixelSink,
  row::{
  ...
  fn process(&mut self, row: Y212Row<'_>) -> Result<(), Self::Error> {
    ...
    if let Some(plan) = plan.as_ref() {
      ...
      if plan.kind().is_filter() {
        ...
        // luma). Co-sited keeps the fused `y212_*` decode.
        #[cfg(all(feature = "y2xx", feature = "yuv-planar"))]
        {
      ...
      // CHECK-before / SET-after split. Co-sited keeps the fused `y212_*` decode.
      packed_yuv422_triple_resample::<BITS>(
        luma_stream_u16,
        rgb_stream,
        rgb_stream_u16,
